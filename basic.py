...
class FuncDefNode:
    def __init__(self, varNameTok, argNameTok, bodyNode) :
        self.var_name_tok = varNameTok
        self.arg_name_toks = argNameTok
...
class CallNode:
    def __init__(self, nodeToCall, argNodes):
        self.node_to_call = nodeToCall
        self.arg_nodes = argNodes
...
class Parser:
        ...
        if self.current_tok.type == TT_LPAREN:
            ...
            if self.current_tok.type == TT_RPAREN:
                ...
                if res.error:
                    return res.failure(InvalidSyntaxError(
                        self.current_tok.pos_start, self.current_tok.pos_end,
                        "Expected ')', 'VAR', 'IF', 'FOR', 'WHILE', 'Void', int, float, identifier"
                    ))
    ...
    # Rules for power operator, ifelse statement,... , looking only for int and float or identifer value
    def atom(self):
        res = ParseResult()
        ...
        return res.failure(InvalidSyntaxError(
            ...
            "Expected int, float, identifier, '+', '-', '(', 'IF', 'FOR', 'WHILE', 'Void'"
        ))
    ...
    def expr(self):
        ...
        # Check if the current token match the token type VAR, use the rule: KEYWORD: VAR IDENTIFIER EQ expr
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected 'VAR', 'IF', 'FOR', 'WHILE', 'Void', int, float, identifier, '+', '-', '(' or 'NOT'"
            ))
    ...
    def func_def(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD,'Void'):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'Void'"
            ))
        ...
        if self.current_tok.type != TT_ACCOLADE:
            return res.failure(InvalidSyntaxError(
                    self.current_tok.pos_start, self.current_tok.pos_end,
                    f"Expected '{ ', '}'"
                ))
...
# Keep track all variables name and their values
...
class Interpreter:
    ...
    def visit_FuncDefNode(self, node, context):
        ...
        funs_value = Function(func_name, body_node, arg_names).set_context(context).set_pos(node.pos_start, node.pos_end)

        if node.var_name_tok:
            context.symbol_table.set(func_name, funs_value)
        
        return res.success(funs_value)
